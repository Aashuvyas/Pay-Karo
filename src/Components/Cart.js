...
export const Cart = () => {
    ...
    const payWithWalletHandler = () => {
        ...
        fs.collection('userCardInfo').doc(user.uid).get().then((snapshot) => {
            ...
            if (parseInt(userData.Amount) < totalPrice) {
                alert('Insufficient funds')
                return
            }
            else {
                fs.collection('userCardInfo').doc(user.uid).update({ Amount: parseInt(userData.Amount) - totalPrice })
                    .then(() => toast.success("Payment done using wallet."))
            ...
            {cartProducts.length > 0 && (
                <div className='container-fluid'>
                    ...
                    <div className='summary-box'>
                        ...
                        <br></br>
                        <button className='btn btn-secondary btn-md' onClick={payWithWalletHandler} >Pay with wallet</button>
