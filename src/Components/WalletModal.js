...
export const WalletModal = ({ TotalPrice, totalQty, hideModal }) => {
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        auth.onAuthStateChanged(user => {
            var newamt = parseInt(amt) - parseInt(TotalPrice);
            console.log(newamt);
            fs.collection('userCardInfo').doc(user.uid).update({ Amount: newamt })
        })
    }
    ...
    return (
        <div className='shade-area'>
            <div className='modal-container'>
                <form className='form-group' onSubmit={CallingFunction}>
