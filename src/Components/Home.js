...
export const Home = (props) => {
    ...
    useEffect(() => {
        if (!products.length || !searchText.length) {
            setSearchProducts([])
            return
        }

        const searchTxt = searchText.toLowerCase()
        const searchResults = [...products].filter(product => product.title.toLowerCase().search(searchTxt) !== -1 || product.description.toLowerCase().search(searchTxt) !== -1)
    ...
    return (
        <>
            ...
            {category.length === 0 && <div className="searchBar" style={{ margin: '15px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <input type="text" style={{ width: '50vw', padding: '5px' }} className='form-control' placeholder='Enter search text' onChange={e => setSearchText(e.target.value)} required />
            ...
            <div className='container-fluid filter-products-main-box'>
                ...
                {searchProducts.length > 0 &&
                    <div className='my-products'>
                        ...
                        <div className='products-box'>
                            <Products products={searchProducts} addToCart={addToCart} />
                        </div>
                    </div>
                }
                {filteredProducts.length > 0 && searchProducts.length === 0 && (
                    <div className='my-products'>
                ...
                {filteredProducts.length < 1 && searchProducts.length === 0 && (
                    <>
                        {products.length > 0 && (
                            <div className='my-products'>
